...
def domain_to_str(bytes src):
    ...
    if isinstance(s, str):
        ...
        return s.decode('ascii') #FIXME mmcniece, shoud this be utf-8
...
def str_to_rrclass(str src):  # mmcniece new and needs conversion
    """
    str_to_rrclass(src)
...
def str_to_rcode(str src):  # mmcniece new and needs conversion
    """
    str_to_rcode(src)
    ...
    cdef uint16_t rval
    cdef wdns_res res
    encoded_src = src.encode('ascii') #FIXME, utf8?
    res = wdns_str_to_rcode(encoded_src, &rval)
    if res != wdns_res_success:
        raise Exception, 'wdns_str_to_rcode() failed: %s' % wdns_res_to_str(res)
    return rval

def str_to_rdata(bytes s, uint16_t rrtype, uint16_t rrclass):  # mmcniece new and needs conversion
    """
...
def str_to_name_case(str src):
    ...
    encoded_src = src.encode('ascii')
    res = wdns_str_to_name_case(encoded_src, &name)
    if res != wdns_res_success:
        raise Exception, 'wdns_str_to_name() failed'
    try:
        s = name.data[:name.len]
